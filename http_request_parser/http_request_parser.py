...
# Notice: methods with names starting with '_' are private methods.

def get_metohd_type_of_raw_http_request(raw_http_request):
...
def get_headers_map_of_raw_http_request(raw_http_request):
    splitted_http_request = raw_http_request.splitlines()
    list_of_raw_headers = splitted_http_request[1:raw_http_request.find('\r\n\r\n')]
    return _process_list_of_raw_headers(list_of_raw_headers)


def get_user_agent_header_of_raw_http_request(raw_http_request):
...
def get_host_header_of_raw_http_request(raw_http_request):
    for http_request_line in raw_http_request.split('\n'):
...
def is_raw_http_request_persistent(raw_http_request):
...
def get_tuple_of_raw_http_request_range_header(raw_http_request, range_low_bound, range_high_bound):
    for http_request_line in raw_http_request.split('\n'):
...
def _process_list_of_raw_headers(list_of_raw_headers):
